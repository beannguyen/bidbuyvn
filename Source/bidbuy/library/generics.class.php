...
class Generic extends Connect
{
    ...
    public function getOption($option, $check = false, $profile = false, $permission = false, $id = '') {
        ...
        if ( $profile )
        {
            $sql = "SELECT `meta_value` FROM `".DB_PRE."members_meta` WHERE `meta_key` = '$option' AND `user_id` = $id LIMIT 1;";
        } elseif ( $permission )
        {
            $sql = "SELECT `permission` FROM `".DB_PRE."login_permissions` WHERE `action` = '$option' AND `level_id` = $id LIMIT 1;";
        }else
        {
            $sql = "SELECT option_value FROM ".DB_PRE."options WHERE option_name = '".$option."' LIMIT 1;";
        }
